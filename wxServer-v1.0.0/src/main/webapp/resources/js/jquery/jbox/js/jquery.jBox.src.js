...
(function ($) {
    $.jBox = function (content, options) {
        ...
        var isIE6 = ($.support.msie && parseInt($.support.version) < 7);
        ...
        var ieButtonFix = $.support.msie ? 'line-height:19px;padding:0px 6px 0px 6px;' : 'padding:0px 10px 0px 10px;';
        ...
        var setFade = function () {
            if (options.showFade) {
                $.jBox.FadeBoxCount++;
                $($.support.msie ? 'html' : 'body').attr('style', 'overflow:hidden;padding-right:17px;');
        ...
        var removeBox = function () {
            if (options.showFade) {
                if ($.jBox.FadeBoxCount == 1) {
                    $($.support.msie ? 'html' : 'body').removeAttr('style');
                }
                $.jBox.FadeBoxCount--;
            }
    ...
    $.jBox.prompt = function (content, title, icon, options) {
        ...
        var minHeight = options.icon == 'none' ? 'height:auto;' : 'min-height:30px;' + (($.support.msie && parseInt($.support.version) < 7) ? 'height:auto !important;height:100%;_height:30px;' : 'height:auto;');
        var html = [];
        html.push('html:');
        ...
        html.push('<span style="position:absolute;left:55px;top:40px;">');
